pipeline {
	...
	stages {
		...
		stage('test') {
			steps {
				bat "\"c:/Program Files (x86)/NUnit.org/nunit-console/nunit3-console.exe\" SchiffeVersenken.Tests/bin/Debug/SchiffeVersenken.Tests.dll --result=TestR.xml;format=nunit2"
			}
		}
	}
